...
def imshow_keypoints(img,
    ...
    Args:
            ...
            thickness (int): Thickness of lines.
    """
    #draw keypoint colors, skeleton, and lines for each region
    pose_kpt_color = []
    
    pose_kpt_color.extend([[255, 0, 0] for i in range(4)])
    pose_kpt_color.extend([[0, 255, 0] for i in range(8)])
    pose_kpt_color.extend([[0, 0, 255] for i in range(8)])
    pose_kpt_color.extend([[0, 255, 255] for i in range(8)])
    pose_kpt_color.extend([[255, 0, 255] for i in range(4)])
    pose_kpt_color.extend([[255, 255, 0] for i in range(5)])
    pose_kpt_color.extend([[255, 255, 255] for i in range(8)])
    pose_kpt_color.extend([[0, 0, 0] for i in range(5)])
    pose_kpt_color.extend([[5, 5, 100] for i in range(8)])
    skeleton = [[0, 1], [1, 2], [2, 3], [3, 0], [4, 5], [5, 6], [6, 7], [7, 4], [8, 9], [9, 10], [10, 11], [11, 8], [12, 13], [13, 14], [14, 15], [15, 12], 
    [16, 17], [17, 18], [18, 19], [19, 16], [20, 21], [21, 22], [22, 23], [23, 20], [24, 25], [25, 26], [26, 27], [27, 24], [28, 29], [29, 30], [30, 31], [31, 28], 
    [32, 33], [33, 34], [34, 35], [35, 36], [36, 32], [37, 38], [38, 39], [39, 40], [40, 37], [41, 42], [42, 43], [43, 44], [44, 41], 
    [45, 46], [46, 47], [47, 48], [48, 49], [49, 45], [50, 51], [51, 52], [52, 53], [53, 50], [54, 55], [55, 56], [56, 57], [57, 54]]
    pose_link_color = []
    pose_link_color.extend([[255, 0, 0] for i in range(4)])
    pose_link_color.extend([[0, 255, 0] for i in range(8)])
    pose_link_color.extend([[0, 0, 255] for i in range(8)])
    pose_link_color.extend([[0, 255, 255] for i in range(8)])
    pose_link_color.extend([[255, 0, 255] for i in range(4)])
    pose_link_color.extend([[255, 255, 0] for i in range(5)])
    pose_link_color.extend([[255, 255, 255] for i in range(8)])
    pose_link_color.extend([[0, 0, 0] for i in range(5)])
    pose_link_color.extend([[5, 5, 100] for i in range(8)])
    """
    pose_kpt_color.extend([[255, 255, 255] for i in range(len(pose_result[0]))])
    pose_link_color = None
    """
    img = mmcv.imread(img)
    ...
    for kpts in pose_result:
        ...
        if pose_kpt_color is not None:
            assert len(pose_kpt_color) == len(kpts), (len(pose_kpt_color), len(kpts))
